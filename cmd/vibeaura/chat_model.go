...
func initialModel(b *brain.Brain) *model {
	...
	if resumeStateFile != "" {
		...
		// Priority 2: Persistent Session State (based on CWD)
		sessionID := b.GetSessionID()
		var stateData []byte
		if err := b.RecallState(sessionID, &stateData); err == nil && len(stateData) > 0 {
			var state chatState
	...
	if len(m.messages) == 1 {
		...
		m.messages = append(m.messages, subtleStyle.Render("Session: ")+aiStyle.Render(cwd))
	}
